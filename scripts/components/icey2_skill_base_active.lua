...
local Icey2SkillBase_Active = Class(function(self, inst)
    self.inst = inst
...
function Icey2SkillBase_Active:_IsRiding()
    return self.inst.components.rider and self.inst.components.rider:IsRiding()
end

function Icey2SkillBase_Active:_CanCost(x, y, z, target)
...
function Icey2SkillBase_Active:_ApplyCost(x, y, z, target)
    for name, v in pairs(self.costs) do
        if DefaultCostDefines[name] and type(v) == "number" then
...
function Icey2SkillBase_Active:IsCoolingDown()
    return GetTime() - self.last_cast_time < self.cooldown
end

function Icey2SkillBase_Active:CanCast(x, y, z, target)
...
function Icey2SkillBase_Active:Cast(x, y, z, target)
    self:_ApplyCost(x, y, z, target)
    self.last_cast_time = GetTime()
end

function Icey2SkillBase_Active:OnSave()
    local data = {}
    if self:IsCoolingDown() then
...
function Icey2SkillBase_Active:OnLoad(data)
    if data ~= nil then
